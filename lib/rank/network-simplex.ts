...
function initLowLimValues(tree: Graph, root: string = tree.nodes()[0]) {
...
function leaveEdge(tree: Graph) {
  return _.find(tree.edges(), function (e: Edge) {
...
function enterEdge(t: Graph, g: Graph, edge: Edge) {
  ...
  const candidates = _.filter(g.edges(), function (edge: Edge) {
    return (
