...
export function run(g: Graph) {
  ...
  const height = _.max(_.values(depths))! - 1 // Note: depths is an Object not an array
...
export function cleanup(g: Graph) {
  ...
  g.removeNode((graphLabel as any).nestingRoot)
  delete (graphLabel as any).nestingRoot
  _.forEach(g.edges(), function (e) {
    const edge = g.edge(e)
    if (edge.nestingEdge) {
      g.removeEdge(e)
