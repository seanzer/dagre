...
function runLayout(g: Graph, time: typeof utilTime) {
...
function makeSpaceForEdgeLabels(g: Graph) {
  const graph = g.graph()
  ;(graph as any).ranksep /= 2
  _.forEach(g.edges(), function (e) {
    const edge = g.edge(e)
    edge.minlen *= 2
    if (edge.labelpos.toLowerCase() !== 'c') {
      if ((graph as any).rankdir === 'TB' || (graph as any).rankdir === 'BT') {
...
function translateGraph(g: Graph) {
  ...
  const marginX = (graphLabel as any).marginx || 0
  const marginY = (graphLabel as any).marginy || 0
  ...
  ;(graphLabel as any).width = maxX - minX + marginX
  ;(graphLabel as any).height = maxY - minY + marginY
}
...
function removeBorderNodes(g: Graph) {
  _.forEach(g.nodes(), function (v) {
    if (g.children(v).length) {
      ...
      const l = g.node(_.last(node.borderLeft) as string)
      const r = g.node(_.last(node.borderRight) as string)
