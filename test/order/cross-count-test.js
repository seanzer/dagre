...
describe("crossCount", function() {
  ...
  it("returns 0 for a layering with no crossings", function() {
    g.setEdge("a1", "b1");
    g.setEdge("a2", "b2");
    expect(crossCount(g, [["a1", "a2"], ["b1", "b2"]])).toEqual(0);
  });

  it("returns 1 for a layering with 1 crossing", function() {
    g.setEdge("a1", "b1");
    g.setEdge("a2", "b2");
    expect(crossCount(g, [["a1", "a2"], ["b2", "b1"]])).toEqual(1);
  });
  ...
  it("returns a weighted crossing count for a layering with 1 crossing", function() {
    ...
    expect(crossCount(g, [["a1", "a2"], ["b2", "b1"]])).toEqual(6);
  });

  it("calculates crossings across layers", function() {
    g.setPath(["a1", "b1", "c1"]);
    g.setPath(["a2", "b2", "c2"]);
    expect(crossCount(g, [["a1", "a2"], ["b2", "b1"], ["c1", "c2"]])).toEqual(2);
  });
  ...
  it("works for graph #1", function() {
    ...
    g.setPath(["a", "f", "i"]);
    g.setEdge("a", "e");
    expect(crossCount(g, [["a", "d"], ["b", "e", "f"], ["c", "i"]])).toEqual(1);
    expect(crossCount(g, [["d", "a"], ["e", "b", "f"], ["c", "i"]])).toEqual(0);
  });
});
